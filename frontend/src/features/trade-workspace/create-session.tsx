...
export function CreateTradeSession({ onCreated }: { onCreated: (session: TradeSession) => void }) {
  ...
  return <form onSubmit={submit} className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm" aria-label="Buat sesi trading">
    <h2 className="text-lg font-semibold">Buat Sesi Baru</h2>
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-medium">Kode Saham<input value={ticker} onChange={e => setTicker(e.target.value)} className="mt-1 w-full rounded-lg border p-2" /></label>
      <label className="text-sm font-medium">Nama Perusahaan<input value={company} onChange={e => setCompany(e.target.value)} className="mt-1 w-full rounded-lg border p-2" /></label>
    </div>
    <label className="mt-4 block text-sm font-medium">Catatan (opsional)<textarea value={note} onChange={e => setNote(e.target.value)} className="mt-1 min-h-20 w-full rounded-lg border p-2" /></label>
