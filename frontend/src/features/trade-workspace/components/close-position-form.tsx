...
export function ClosePositionForm({ closePrice, closeTimestamp, closeReason, closeNote, busy, onSubmit, onPriceChange, onTimestampChange, onReasonChange, onNoteChange, onCancel }: {
  ...
  return (
    <form onSubmit={onSubmit} className="overflow-hidden rounded-[var(--radius-large)] border border-[var(--color-status-warning)] bg-[var(--color-surface-factual)] shadow-[var(--elevation-low)]">
      ...
      <div className="flex min-w-0 flex-col gap-2 border-t border-[var(--color-border-default)] bg-[var(--color-elevated-background)] px-[var(--space-card)] py-[var(--space-4)] sm:flex-row sm:justify-end">
        <button type="button" disabled={busy} onClick={onCancel} className="min-h-11 w-full min-w-0 rounded-[var(--radius-compact)] border border-[var(--color-border-strong)] bg-[var(--color-surface-factual)] px-4 py-2.5 text-[var(--text-size-compact-body)] font-semibold text-[var(--color-text-default)] disabled:opacity-50 sm:w-auto">Batal</button>
        <button type="submit" disabled={busy} className="min-h-11 w-full min-w-0 rounded-[var(--radius-compact)] bg-[var(--color-text-strong)] px-4 py-2.5 text-[var(--text-size-compact-body)] font-semibold text-[var(--color-text-inverse)] disabled:opacity-50 sm:w-auto">{busy ? "Menyimpan…" : "Konfirmasi Tutup Posisi"}</button>
      </div>
    </form>
  );
}
