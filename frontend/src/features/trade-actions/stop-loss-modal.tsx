...
async function loadAIProposal(sessionId: string, signal: AbortSignal): Promise<string | null> {
  try {
    ...
    const sl = p.stop_loss_assessment as Record<string, unknown> | undefined;
    if (sl?.proposed_stop_loss != null) return String(sl.proposed_stop_loss);
    return null;
  } catch {
    return null;
  }
...
function validate(value: string): string | null {
  if (!value.trim()) return "Nilai stop loss wajib diisi.";
  const num = parseFloat(toApiNumber(value));
  if (isNaN(num) || num <= 0) return "Stop loss harus lebih besar dari 0.";
  return null;
}
...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  action: "CONFIRM_STOP" | "CHANGE_STOP";
  activeStopLoss: string | null;
}

export function StopLossModal({ sessionId, isOpen, onClose, onSuccess, action, activeStopLoss }: Props) {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="mx-4 w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
        <StopLossForm key={sessionId + action} sessionId={sessionId} onClose={onClose} onSuccess={onSuccess} action={action} activeStopLoss={activeStopLoss} />
      </div>
    </div>
  );
}

function StopLossForm({ sessionId, onClose, onSuccess, action, activeStopLoss }: { sessionId: string; onClose: () => void; onSuccess: () => void; action: "CONFIRM_STOP" | "CHANGE_STOP"; activeStopLoss: string | null }) {
  ...
  useEffect(() => {
    ...
    loadAIProposal(sessionId, ctrl.signal).then((p) => {
      ...
      setProposal(p);
      setLoading(false);
      setValue(p ?? activeStopLoss ?? "");
    }).catch(() => {
      if (!ctrl.signal.aborted) {
        setLoading(false);
        setValue(activeStopLoss ?? "");
      }
    });
    return () => { ctrl.abort(); cancelledRef.current = true; };
  }, [sessionId, activeStopLoss]);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      const api = action === "CONFIRM_STOP" ? confirmStop : changeStop;
      await api({ session_id: sessionId, idempotency_key: generateIdempotencyKey(), stop_loss: toApiNumber(value), confirmed_at: nowISO(), note: note.trim() || null });
      ...
      else setSubmitState({ type: "error", message: "Gagal memperbarui stop loss. Silakan coba lagi." });
    }
  ...
  return (
    <><div className="mb-4 flex items-center justify-between">
      <h2 className="text-lg font-semibold text-zinc-800">
        {action === "CONFIRM_STOP" ? "Konfirmasi Stop Loss" : "Ubah Stop Loss"}
      </h2>
    ...
    <div className="mb-4 space-y-1">
      <p className="text-sm text-zinc-500">
        <span className="font-medium text-zinc-700">Stop Loss Aktif: </span>
        {activeStopLoss ? currency(parseFloat(activeStopLoss)) : "—"}
      </p>
      {proposal != null && (
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-zinc-700">
          Nilai Stop Loss <span className="text-red-500">*</span>
        </label>
        <input
      ...
      <div className="flex justify-end gap-3">
        ...
        <button type="submit" disabled={submitState === "pending"}
          ...
          {submitState === "pending" ? "Memproses…" : (action === "CONFIRM_STOP" ? "Konfirmasi Stop Loss" : "Ubah Stop Loss")}
        </button>
      </div>
    </form>
    </>
