...
export function WaitUpdateActionRoute({ sessionId }: { sessionId: string }) {
  ...
  const isEligible =
    ...
    currentStep.workflow_actions.includes("SUBMIT_WAIT_UPDATE");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await uploadWaitUpdateInput(reqSessionId, {
      ...
      await submitWaitUpdateAnalysis(reqSessionId);
      ...
      if (isCurrentMutation(reqSessionId, reqRouteGen, reqMutGen)) {
        setErrorFeedback(
          "WAIT Update belum dapat dikirim. Periksa kembali data yang dimasukkan lalu coba lagi.",
        );
      }
    } finally {
      if (isCurrentMutation(reqSessionId, reqRouteGen, reqMutGen)) {
          ...
          {!isEligible ? (
            <div className="mt-6 min-w-0 space-y-3 border-t border-[var(--color-border-default)] pt-5">
              <h1 className="text-xl font-bold text-[var(--color-text-strong)]">
                WAIT Update tidak tersedia
              </h1>
              <p className="break-words text-sm text-[var(--color-text-muted)]">
                Sesi ini tidak dapat menerima WAIT Update pada tahap saat ini.
              </p>
              <Link
                href={backHref}
            ...
            >
              <div>
                <h1 className="text-xl font-bold text-[var(--color-text-strong)]">
                  Formulir Pembaruan WAIT
                </h1>
                <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                  Unggah orderbook terbaru, tambahkan Broker Flow 1D bila tersedia, dan masukkan informasi harga saat ini untuk melanjutkan pemantauan.
                </p>
              </div>

              {validationError ? (
                <p id="wait-validation-error" role="alert" className="break-words text-sm font-medium text-[var(--color-status-danger)]">
                  {validationError}
                </p>
              ) : null}
              ...
              {errorFeedback ? (
                <div role="alert" className="rounded-[var(--radius-compact)] border border-[var(--color-status-danger)] bg-[var(--color-status-danger-subtle)] p-3 text-sm text-[var(--color-status-danger)]">
                  <p className="font-bold">WAIT Update belum dapat dikirim</p>
