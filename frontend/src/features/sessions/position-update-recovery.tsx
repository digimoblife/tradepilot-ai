...
import type { CurrentStep, PositionUpdateItem } from "@/features/trade-workspace/types";
...
function presentationOf(
  updateItem: PositionUpdateItem,
  step: CurrentStep,
  candidateId: string | null,
): PresentationState {
  if (!candidateId || updateItem.analysis_request_id !== candidateId) {
    return "UNAVAILABLE";
  }
  if (updateItem.analysis_type !== "POSITION_UPDATE") return "UNAVAILABLE";
  if (updateItem.request_status === "PENDING" || updateItem.request_status === "PROCESSING") {
    return "PROCESSING";
  }
  if (updateItem.request_status === "COMPLETED") return "COMPLETED";
  if (updateItem.request_status === "FAILED") {
    const canRetry =
      !step.read_only &&
      (step.workflow_actions.includes("SUBMIT_POSITION_UPDATE") ||
        (step.failed_request?.analysis_type === "POSITION_UPDATE" &&
          step.failed_request.retry_allowed));
    return canRetry ? "FAILED_RETRY_AVAILABLE" : "FAILED_READ_ONLY";
  }
  return "UNAVAILABLE";
}

export function PositionUpdateRecovery({
  sessionId,
  step,
  refetch,
}: {
  ...
  const [updateItem, setUpdateItem] = useState<PositionUpdateItem | null>(null);
  ...
  const candidate =
    active?.analysis_type === "POSITION_UPDATE"
      ? active
      : failed?.analysis_type === "POSITION_UPDATE"
        ? failed
        : null;
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const res = await readPositionUpdates(sessionId, controller.signal);
        if (current !== generation.current) return;

        const match = res.updates.find((u) => u.analysis_request_id === candidate?.id) ?? null;
        ...
        if (match.request_status === "COMPLETED" || match.request_status === "FAILED") {
          await refetch();
          return;
        }
      } catch {
  ...
  const state = updateItem
    ? presentationOf(updateItem, step, candidate?.id ?? null)
    : shouldRecover
      ? "PROCESSING"
      : "IDLE";
      ...
      >
        <div className="min-w-0 rounded-[var(--radius-large)] border border-[var(--color-status-danger)] bg-[var(--color-status-danger-subtle)] p-[var(--space-card)] shadow-[var(--elevation-low)] sm:p-6">
          <h2 className="text-xl font-bold leading-[var(--text-line-heading)] text-[var(--color-status-danger)]">
            Status Position Update belum dapat dimuat
          </h2>
          <p className="mt-2 break-words text-sm text-[var(--color-text-default)]">
            Status Position Update belum dapat dimuat. Silakan periksa kembali.
          </p>
          <button
            type="button"
      ...
      >
        <div className="min-w-0 rounded-[var(--radius-large)] border border-[var(--color-status-processing)] bg-[var(--color-status-processing-subtle)] p-[var(--space-card)] shadow-[var(--elevation-low)] sm:p-6">
          <h2 className="text-xl font-bold leading-[var(--text-line-heading)] text-[var(--color-text-strong)]">
            Position Update sedang diproses
          </h2>
          <p className="mt-2 break-words text-sm text-[var(--color-text-default)]">
            Data posisi terbaru sedang dianalisis. Hasilnya akan tersedia setelah proses selesai.
          </p>
    ...
    return (
      ...
      >
        <div className="min-w-0 rounded-[var(--radius-large)] border border-[var(--color-status-success)] bg-[var(--color-status-success-subtle)] p-[var(--space-card)] shadow-[var(--elevation-low)] sm:p-6">
          ...
          <p className="mt-2 break-words text-sm text-[var(--color-text-default)]">
            Hasil analisis posisi terbaru sudah tersedia.
          </p>
          <Link
  ...
  return (
    ...
    >
      <div className="min-w-0 rounded-[var(--radius-large)] border border-[var(--color-status-danger)] bg-[var(--color-status-danger-subtle)] p-[var(--space-card)] shadow-[var(--elevation-low)] sm:p-6">
        <h2 className="text-xl font-bold leading-[var(--text-line-heading)] text-[var(--color-status-danger)]">
          Position Update belum berhasil
        </h2>
        <p className="mt-2 break-words text-sm text-[var(--color-text-default)]">
          Analisis Position Update belum dapat diselesaikan.
        </p>
