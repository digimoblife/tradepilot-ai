...
class WaitUpdateAnalysisReadNotFoundError(WaitUpdateAnalysisReadError):
    pass


@dataclass(frozen=True, slots=True)
class WaitUpdateAnalysisReadResult:
    ...
    async def get_latest(
        ...
        *,
        ...
        if trade_session is None:
            raise WaitUpdateAnalysisReadNotFoundError("Rebuild session was not found")
        ...
        if request is None:
            raise WaitUpdateAnalysisReadNotFoundError(
        ...
        return WaitUpdateAnalysisReadResult(
            ...
            error_code=request.error_code[:64] if is_failed and request.error_code else None,
            error_message=(
                request.error_message[:500]
                if is_failed and request.error_message
                else None
            ),
