...
async def _make_session(
    engine: AsyncEngine,
    owner_id: uuid.UUID,
    status: TradeSessionStatus = TradeSessionStatus.DRAFT,
...
class TestTransitionRegistry:
    ...
    def test_draft_to_open_invalid(self) -> None:
        assert not is_transition_allowed(
            TradeSessionStatus.DRAFT, TradeSessionStatus.OPEN_POSITION
        )
    ...
    def test_partial_to_partial_allowed(self) -> None:
        assert is_transition_allowed(
            TradeSessionStatus.PARTIALLY_CLOSED, TradeSessionStatus.PARTIALLY_CLOSED
        )
...
class TestLifecycleService:
        ...
        async with factory() as s:
            ...
            with pytest.raises(InvalidSessionTransitionError) as exc:
                await svc.transition(
                    session_id=sid,
                    owner_id=uid,
                    target_status=TradeSessionStatus.OPEN_POSITION,
                )
    ...
    async def test_state_unchanged_after_invalid(
        ...
        async with factory() as s:
            ...
            with pytest.raises(InvalidSessionTransitionError):
                await svc.transition(
                    session_id=sid,
                    owner_id=uid,
                    target_status=TradeSessionStatus.DRAFT,
                )
...
class TestAnalyzing:
    async def test_enter_analyzing_preserves_stable(
        self, engine: AsyncEngine, user_id: uuid.UUID
    ) -> None:
    ...
    async def test_analyzing_from_open_position(
        self, engine: AsyncEngine, user_id: uuid.UUID
    ) -> None:
