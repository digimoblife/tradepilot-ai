...
def test_market_facts_is_none_when_absent_from_snapshot() -> None:
    result = RebuildAnalysisContextBuilder._market_facts(  # type: ignore[arg-type]
        SimpleNamespace(input_snapshot={"ticker": "BBRI"})
    )
    assert result is None


def test_market_facts_is_none_when_snapshot_is_malformed() -> None:
    for malformed in ({"market_facts": "not-a-dict"}, {"market_facts": None}):
        result = RebuildAnalysisContextBuilder._market_facts(  # type: ignore[arg-type]
            SimpleNamespace(input_snapshot=malformed)
        )
        assert result is None


def test_market_facts_parses_a_fully_populated_snapshot() -> None:
    request = SimpleNamespace(
        input_snapshot={
            "market_facts": {
                ...
                "pbv_ratio": 1.8,
                "market_cap": 5.0e13,
                "volume_shares_today": 1_000_000,
                "avg_volume_20d_shares": 500_000.0,
                "volume_vs_average_ratio": 2.0,
                "index_name": "IHSG",
                "index_change_percent": 0.42,
                "index_trend": "BULLISH",
                "foreign_status": "ACCUMULATION",
                "foreign_flow_1m": {"net_shares": 1_000_000, "net_value_idr": 5_000_000_000.0},
                "foreign_flow_3m": {"net_shares": -200_000, "net_value_idr": -900_000_000.0},
                "avg_daily_value_idr_20d": 2_500_000_000.0,
                "system_spread_percent": 0.25,
                "system_bid_ask_ratio": 1.45,
                "system_total_bid_lots": 125_000,
                "system_total_ask_lots": 86_000,
                "ma20": 4200.0,
                "ma50": 4100.0,
                "ma200": 3900.0,
                "rsi14": 58.5,
    ...
    result = RebuildAnalysisContextBuilder._market_facts(request)  # type: ignore[arg-type]
    assert result is not None
    assert result.pe_ratio == 12.5
    assert result.index_name == "IHSG"
    assert result.foreign_flow_1m is not None
    assert result.foreign_flow_1m.net_shares == 1_000_000
    assert result.foreign_flow_3m is not None
    assert result.foreign_flow_3m.net_value_idr == -900_000_000.0
...
def test_market_facts_tolerates_partial_payload() -> None:
    request = SimpleNamespace(
        input_snapshot={"market_facts": {"pe_ratio": 9.1, "index_trend": None}}
    )
    result = RebuildAnalysisContextBuilder._market_facts(request)  # type: ignore[arg-type]
    assert result is not None
    assert result.pe_ratio == 9.1
    assert result.index_trend is None
    assert result.foreign_flow_1m is None
    assert result.avg_daily_value_idr_20d is None
    assert result.system_spread_percent is None
    assert result.system_bid_ask_ratio is None
    assert result.system_total_bid_lots is None
    assert result.system_total_ask_lots is None
...
def test_market_facts_is_none_when_a_field_has_the_wrong_type() -> None:
    for invalid_val in (
        {"pe_ratio": {"nested": "object"}},
        {"key_supports": "not-a-sequence"},
        {"key_supports": [{"nested": "object"}]},
        {"system_spread_percent": {"nested": "object"}},
        {"system_total_bid_lots": "not-an-int"},
    ):
        request = SimpleNamespace(input_snapshot={"market_facts": invalid_val})
        result = RebuildAnalysisContextBuilder._market_facts(request)  # type: ignore[arg-type]
        assert result is None
