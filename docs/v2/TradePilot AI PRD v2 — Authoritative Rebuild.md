...
# 27a. Rebuild Queue Runtime Architecture

The rebuild uses PostgreSQL database polling on `analysis_requests_v2` as its authoritative and durable queue source.

## Backend Submission Requirements:
1. Validate ownership, lifecycle eligibility, and evidence.
2. Reject duplicate active requests.
3. Create exactly one `analysis_requests_v2` row with status `PENDING`.
4. Assign selected evidence to the request.
5. Transition the session to `ANALYZING` within the approved transaction boundary.
6. Commit durable database state.
7. Return HTTP 202.
8. Do not publish to another transport or queue infrastructure.

## Worker Processing Requirements:
1. Poll `analysis_requests_v2` using the configured worker polling interval.
2. Atomically claim one eligible `PENDING` request using PostgreSQL locking.
3. Ensure two workers cannot process the same request concurrently.
4. Mark the request `PROCESSING`.
5. Run the approved rebuild processing flow.
6. Call Gemini once.
7. Store raw and processed responses.
8. Mark the request `COMPLETED` or `FAILED`.
9. Transition the session to its approved resulting lifecycle status.
10. Preserve failed requests for explicit retry.

## Queue Guarantees:
- durable across backend and worker restarts;
- one active request per approved uniqueness rule;
- at-most-one active worker claim;
- no hidden retry;
- no legacy lifecycle delegation;
...
- No in-process queue;
- No reuse of legacy `analysis_jobs`.
